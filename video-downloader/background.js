...
/*
	...
	*/
    checkObject:function(details)
    {
		...
		if(!url.includes("https://hls-c.udemycdn.com/")) {
			return;
		}
        ...
        for (var i = 0; i < details.responseHeaders.length; ++i) 
        {
            ...
            else if (details.responseHeaders[i].name.toLowerCase() === 'content-length')
                len = parseInt(details.responseHeaders[i].value); 
            else if (!len && details.responseHeaders[i].name.toLowerCase() === 'content-range')
                len = parseInt(details.responseHeaders[i].value);
        }
		//console.log(url.includes("https://hls-c.udemycdn.com/"));
		...
		//if ( len<1024)
		//	return;
		var ext = ["m3u8","ts","flv","mp4","mov","m4v","webm","mpg","mp3","aac","mp2t","video/mp2t","video"];
		var isVideo = false;
        ...
        if (isVideo != false)
        {
			...
			// var options = { url:item.url, filename:"testfile.ts", saveAs:true};    				
